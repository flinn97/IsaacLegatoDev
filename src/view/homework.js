        ...
        return (
            <div style={
                ...
                <div style={
                    ...
                    {bool && (<Addhomework app={app} handleClose={dispatch.bind(this, { popupSwitch: "" })} />)}
                    <div style={{
                        display: "flex",
                        flexDirection: "row",
                        ...
                        >Practice</div>
                        {this.props.app.state.currentuser.getJson().role === "student" ? (<></>) : (<button className="btn  btn-block"
                            onClick={() => { dispatch({ popupSwitch: "addhomework", operate: 'addhomework', object: { owner: id } }) }}
                            style={

                                {...styles.buttons.buttonAdd,
                                marginRight: window.innerWidth<1300 ? "":"-"+styles.margins.margin3,
                            }
                            }
                            ...
                            {comp.getList("homework", id).map((homework, index) =>
                                <div style={{
                                    ...
                                    <div style={{
                                            ...
                                            fontWeight: styles.fonts.fontweightMain,
                                        }}>Notes: {homework.getJson().description}
                                        </div>
                                    </div>)}
                        </div></div>
            ...
            <button  className="btn  btn-block" onClick={state.currentstudent?.clearTime} style={styles.buttons.buttonClear}>Clear Time</button></div>)} 
                            ...
                            <p style={{marginBottom:"15px", fontSize:"19px"}}>Current Practice Progress</p>
                            <Checkedd big={true} size={styles.checkbox.size1} app={app} component={state.currentstudent}
                                ...
                                <div style={{width:"10vw",  height:"50px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:'center',  marginTop:"5vh"}}>
                                    <div style={{display:"flex", flexDirection:"row", alignItems:"center"}}><img style={{marginRight:"10px"}} src={fire} alt= "fire"/><div>
                                    <div style={{fontSize:"17px"}}>{state.currentstudent.getJson().totalDaysPracticed} Day Streak</div>
                                    <div  style={{fontSize:"14px"}}> Keep up the good work!</div></div>
                                    </div></div></>)}
                        </div>
                    </div>
