        ...
        return (
            ...
            <div style={{display:"flex", flexDirection:"column", height:"100%",  display:"flex", alignItems:"center"}}>
               ...
               <div style={{display:"flex", justifyContent:"center", alignItems:"center"}}>
                <img style={{position:'absolute', marginTop:"35px", width:"13vw"}} src={progress} alt="starpoints" /> <div 
                style={{
                    ...
                    marginRight:"3px"}}><div style={{fontSize: window.innerWidth<1300?"12px": "auto"}}>{starpoints?.getJson()?.level}</div><div style={{fontSize: window.innerWidth<1300?"12px": "auto"}}>level</div></div></div>
               ...
               <div style={{display:"flex", flexDirection:"row", justifyContent:"center", alignItems:"center", width:'100%'}}>
                <div style={{display:"flex", flexDirection:"row", flexWrap:"wrap", justifyContent:"center", alignItems:"center", width: state.currentuser.getJson().role==="teacher"?"100%":"80%"}}>
                {badgeList.slice(state.currentuser.getJson().role==="teacher"? badgeList.length-3: badgeList.length-6,badgeList.length).reverse()?.map((badge, index)=>
               <img onClick={async()=>{
                ...
                }} src={badge.getJson().picURL} style={{width: state.currentuser.getJson().role==="teacher"? "4vw": "3.5vw"}}/>)}
               </div>
               </div>
