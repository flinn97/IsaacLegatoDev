                ...
                <div style={{
                    ...
                    <div style={{ height: "18vh" }}>
                        <div className="scroller" id="scroll">
                            {list.map((post, index) =>
                                <div key={index} style={{ display: "flex", flexDirection: 'row', width: "100%", justifyContent: (state.currentuser.getJson().role === "teacher" && !post.getJson().student) ? "flex-end" : "flex-start" }}>
                                    <div style={{maxWidth:"60%"}}>
                                    {state.currentuser.getJson().role === "teacher" && !post.getJson().student?(
                                    <div style={{background:this.props.app.state.styles.colors.colorLink, borderRadius:"7px", padding:"7px", maxWidth:"", color:styles.colors.color6}}>{post.getJson().content}</div>):(<div style={{background:"#D1D1D1", borderRadius:"7px", padding:"7px", color:"black"}}>{post.getJson().content}</div>)}
                                    
                                    </div>
                                </div>
                            )}
                        </div></div>
                    <div className="form-group" style={{ position: "relative", paddingLeft: "20px", paddingRight: "20px", marginBottom: "8px" }}>
                        <input style={{ borderRadius: "20px", height: "35px" }}
